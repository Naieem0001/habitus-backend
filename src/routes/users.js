...
// Add XP to user
router.post('/xp/add', verifyToken_1.verifyToken, async (req, res) => {
    try {
        ...
        const { data: user, error: fetchError } = await supabaseAdmin_1.supabaseAdmin
            .from('users')
        ...
        // Update user
        const { data: updatedUser, error: updateError } = await supabaseAdmin_1.supabaseAdmin
            .from('users')
            .update({
            xp: newXp,
            level: newLevel,
...
// Update streak
router.post('/streak/update', verifyToken_1.verifyToken, async (req, res) => {
    try {
        const userId = req.user.id;
